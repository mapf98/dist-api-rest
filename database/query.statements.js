module.exports = {
  ...
  READ_SCHOOL:
    "SELECT S.id, S.name, S.description, S.created_date, F.name AS Faculty FROM SCHOOL S JOIN FACULTY F ON S.fk_faculty = F.id WHERE S.status = 'ENABLED' AND F.status = 'ENABLED';",
  UPDATE_SCHOOL:
  ...
  READ_SECTION:
    "SELECT SE.id, SE.name, SE.description, SE.uc, SE.semester, SE.type, SE.ht, SE.hp, SE.hl, SE.created_date, SC.name AS School FROM SECTION SE JOIN SCHOOL SC ON SE.fk_school = SC.id WHERE SE.status = 'ENABLED' AND SC.status = 'ENABLED';",
  UPDATE_SECTION:
  ...
  READ_PERSON:
    "SELECT id, dni, first_name, last_name, created_date FROM PERSON WHERE STATUS = 'ENABLED';",
  UPDATE_PERSON:
